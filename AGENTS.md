...
All local `prebuild:*`, `ios*`, and `android*` npm scripts explicitly select `APP_VARIANT=development`. The run scripts reapply the platform Prebuild before `expo run` because Expo skips automatic Prebuild when that native directory already exists. Use these npm scripts instead of invoking `expo run:*` directly. EAS Build does not use the local prebuild scripts: `.easignore` excludes both generated native directories, forcing cloud and `--local` production builds to regenerate them with the production profile and production Firebase file variables. Never remove those native-directory exclusions.
