...
All local `prebuild:*` npm scripts explicitly select `APP_VARIANT=development`. Run the matching Prebuild script before an `ios*` or `android*` run script; the run scripts compile the existing generated native project and do not reapply Expo config plugins themselves. Use a clean Prebuild after changing a native identifier, native dependency, or native configuration. EAS Build does not use the local prebuild scripts: `.easignore` excludes both generated native directories, forcing cloud and `--local` production builds to regenerate them with the production profile and production Firebase file variables. Never remove those native-directory exclusions.
