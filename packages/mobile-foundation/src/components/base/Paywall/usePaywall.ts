...
import {
  ...
  purchasePackage,
  restorePurchases,
} from '@rhysle/mobile-foundation/services/revenueCat'
...
export const usePaywall = ({
  ...
  const handleSubscribe = async () => {
    ...
    setPurchasing(true)
    try {
      const result = await purchasePackage(appConfig, selectedPackage)
  ...
  const handleRestore = async () => {
    ...
    setPurchasing(true)
    try {
      const result = await restorePurchases(appConfig)
      if (result.success) {
