...
export class AppleStoreClient {
  ...
  private territoryIdentifiers: { type: 'territories'; id: string }[] | undefined
  ...
  constructor(
    ...
    this.metadata = new AppleMetadataReconciler(
      ...
      async <T extends JsonApiResource<object>>(pathOrUrl: string) => this.listAll<T>(pathOrUrl)
    )
  }
  ...
  private async listAll<T extends JsonApiResource<object>>(pathOrUrl: string): Promise<T[]> {
    const items: T[] = []
  ...
  private async listAllWithIncluded<T extends JsonApiResource<object>>(
    pathOrUrl: string
  ): Promise<{ data: T[]; included: JsonApiResource<object>[] }> {
    const data: T[] = []
    const included: JsonApiResource<object>[] = []
    let next: string | undefined = pathOrUrl
    while (next) {
      const response: JsonApiListResponse<T> | undefined = await this.request(next)
      if (!response) break
  ...
  private async reconcileFreeTrial(
    ...
    if (this.reporter.command === 'verify') {
      ...
      this.reporter.info(
        `Apple free-trial transition pending: pnpm monetization:activate -- --confirm`
      )
    }
  }
  ...
  private async allTerritoryIdentifiers(): Promise<{ type: 'territories'; id: string }[]> {
