...
interface GoogleRegionalPriceMatrix {
  regionalConfigs: { regionCode: string; price: GoogleMoney }[]
...
export class GooglePlayClient {
  ...
  private priceDifferences(
    currentConfigs: { regionCode: string; price: GoogleMoney; available: boolean }[],
    desiredConfigs: { regionCode: string; price: GoogleMoney }[]
  ): {
    changed: string[]
    increased: string[]
    decreased: string[]
  ...
  private async reconcileSubscriptionPrices(): Promise<void> {
    ...
    for (const key of keys) {
      ...
      const basePlans: GoogleBasePlan[] = subscription.basePlans.map((item) =>
      ...
      const updated: GoogleSubscription | undefined = await this.request<GoogleSubscription>(url, {
        method: 'PATCH',
        body: {
  ...
  private freeTrialOfferDifferences(
    ...
    const normalizeRegions = (
      regions: { regionCode: string; newSubscriberAvailability: boolean }[]
    ...
    ): { regionCode: string; free: boolean }[] =>
      regions
  ...
  private async syncSubscription(): Promise<void> {
    ...
    for (const key of keys) {
      ...
      if (this.reporter.command === 'apply') {
        ...
        const basePlans: GoogleBasePlan[] = [...subscription.basePlans.map(withoutState), newPlan]
        const url = appendQuery(
          `${API_ROOT}/${encodeURIComponent(this.environment.packageName)}/subscriptions/${encodeURIComponent(subscription.productId)}`,
          {
            updateMask: 'basePlans',
        ...
        const updated: GoogleSubscription | undefined = await this.request<GoogleSubscription>(
          url,
          {
            method: 'PATCH',
            body: {
  ...
  private async reconcileFreeTrial(subscription: GoogleSubscription): Promise<void> {
    ...
    if (!trial) {
      ...
      if (active.length === 0) {
        ...
        this.reporter.info(
          'Google free-trial deactivation pending: pnpm monetization:activate -- --confirm'
        )
      }
      return
    }
    ...
    if (!desired) {
      ...
      if (!matches && desired.offer.state !== 'ACTIVE' && this.reporter.command === 'apply') {
        ...
        this.reporter.info(
          'Google live free-trial update pending: pnpm monetization:activate -- --confirm'
        )
        ...
        this.reporter.info(
          'Google free-trial activation pending: pnpm monetization:activate -- --confirm'
        )
      }
    }
    ...
    if (staleActive.length > 0) {
      if (this.reporter.command === 'verify') {
        ...
        this.reporter.info(
          'Google obsolete free-trial deactivation pending: pnpm monetization:activate -- --confirm'
        )
      }
    }
  }
  ...
  private verifyGooglePrice(
    label: string,
    regionalConfigs: { regionCode: string; price: GoogleMoney; available: boolean }[],
    desiredConfigs: { regionCode: string; price: GoogleMoney }[],
