...
import { getAppContext, loadAppModule, resolveAppPackage } from './app-context'

const { FONT_NAME } = loadAppModule<{ FONT_NAME: string }>('src/configs/fonts.ts')
...
function main() {
  ...
  try {
    ...
    if (manifest.dependencies?.[packageName]) {
      try {
        resolveAppPackage(`${packageName}/package.json`)
        alreadyInstalled = true
      } catch {
        /* Install a declared dependency if this checkout has not installed it yet. */
      }
    }
    if (!alreadyInstalled)
      execFileSync('pnpm', ['exec', 'expo', 'install', packageName], {
        cwd: ROOT,
        stdio: 'inherit',
      })
  } catch {
  ...
  const packageDir = path.dirname(resolveAppPackage(`${packageName}/package.json`))
  ...
  for (const { key, suffix, weight } of WEIGHTS) {
    ...
    if (fs.existsSync(nestedPath)) {
      relativePath = path.relative(ROOT, nestedPath)
    } else if (fs.existsSync(flatPath)) {
      relativePath = path.relative(ROOT, flatPath)
    } else {
  ...
  console.log('   Prod: Run `pnpm exec expo prebuild --clean` to embed fonts natively.')
  console.log('')
}
