...
export const DbMeterGauge = ({
  ...
  const gradientDirection = getDbMeterGradientDirection(isRTL)
  ...
  const indicatorStyle = useAnimatedStyle(() => ({
    transform: [
      {
        translateX: getDbMeterIndicatorTranslation(position.value, trackWidth.value, isRTL),
      },
    ],
  }))
...
const createStyles = createThemedStyles((t) => ({
  ...
  indicator: {
    position: 'absolute',
    ...DB_METER_INDICATOR_ANCHOR,
    width: DB_METER_INDICATOR_SIZE,
    height: DB_METER_INDICATOR_SIZE,
    borderRadius: t.borderRadius.full,
    borderWidth: 3,
