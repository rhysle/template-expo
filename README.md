...
Every iOS metadata upload reads the tracked `fastlane/ios/app_store_config.json`, synchronizes the app-level Content Rights declaration, and replaces the App Review attachment with the configured local recording. Set `IOS_APP_REVIEW_ATTACHMENT_PATH` in `.env.fastlane.local` from `.env.fastlane.example` and store the product-specific recording under the gitignored `fastlane/ios/review/` directory. The lane stops before contacting App Store Connect when the tracked config is invalid or the recording is missing. It uploads metadata only and leaves app pricing, App Privacy responses, build selection, and the final App Review submission as manual App Store Connect release steps.
